...
st.markdown(
    "This project is about clustering locations based on the number of cafes, department stores and gyms near apartments around the give latitude and longitude"
)
st.markdown(
    """
    Major project by
...
if st.button("Show map"):
    if not input_long or not input_lat:
        ...
        st.components.v1.html(
            open("map.html", "r", encoding="utf-8").read(), height=600
        )
        st.markdown(
            f"""Legend: The clusters are in the descending order of their distance from the college and number of amenities available in the vicinity.
